...
class Conta:
    ...
    def transferir(self, conta_destino, valor):
        if valor <= self._saldo + self._limite:
            self._saldo -= valor
            self._historico._transacoes.append(f'Transferência {valor} para conta {conta_destino}.')
            conta_destino._saldo += valor
            conta_destino._historico._transacoes.append(f'Transferência {valor} da conta {self}.')
        else:
...
##            return False
    ...
    def __str__(self):
        return f'número: {self._numero}, Saldo: {self._saldo}'
    ...
    def get_listar_clientes(self):
        print(f'\n\nCliente(s) da conta {self}: ')
        print('----------------------------')
        for associacao in self._clientes:
            print(associacao._cliente)
